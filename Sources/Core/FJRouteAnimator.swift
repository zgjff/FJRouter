...
/// 显示匹配路由控制器的动画协议
@MainActor public protocol FJRouteAnimator: Sendable {
    /// 开始路由转场动画
    /// - Parameters:
    ///   - fromVC: 要跳转到的源控制器
    ///   - toVC: 匹配到的路由指向的控制器
    ///   - matchState: 匹配到的路由信息
    func startAnimatedTransitioning(from fromVC: UIViewController?, to toVC: UIViewController, state matchState: FJRouterState)
}
