...
struct FJRouterStoreMatchTests {
    ...
    @Test func testSuccessDoesNotRedirectNoChildHasRedirect() async throws {
        let config = await createConfig(action: { config in
            await config.addRoute(try! FJRoute(path: "/show/:id", builder: self._builder, redirect: FJRouteCommonRedirector(redirect: { _  in .none })))
        })
    ...
    @Test func testSuccessRedirectNoChildHasRedirect() async throws {
        let config = await createConfig(action: { config in
            await config.addRoute(try! FJRoute(path: "/show/:id", builder: self._builder, redirect: FJRouteCommonRedirector(redirect: { _  in "/pages/78" })))
        })
        let p = "/show/123?name=haha&age=18"
        let url = URL(string: p)!
        let result = try await config.match(url: url, extra: 123, ignoreError: false)
            ...
            let r = try! FJRoute(path: "/w", builder: _builder, routes: [
                try! FJRoute(path: "x", builder: _builder, routes: [
                    try! FJRoute(path: "y", builder: _builder, routes: [
                        try! FJRoute(path: "z", builder: _builder, redirect: FJRouteCommonRedirector(redirect: { _ in "/a/b/c" }))
                    ])
                ])
            ])
    ...
    @Test func testErrorHasChildsHasRedirect() async throws {
        let config = await createConfig(action: { config in
            let r = try! FJRoute(path: "/w", builder: _builder, routes: [
                try! FJRoute(path: "x", builder: _builder, routes: [
                    try! FJRoute(path: "y", builder: _builder, routes: [
                        try! FJRoute(path: "z", builder: _builder, redirect: FJRouteCommonRedirector(redirect: { _ in "/a/b/f" }))
                    ])
                ])
            ])
            await config.addRoute(r)
        ...
        let config = await createConfig(action: { config in
            let r1 = try! FJRoute(path: "/user", builder: self._builder, routes: [
                FJRoute(path: "settings", builder: self._builder, routes: [
                    FJRoute(path: "reset", builder: self._builder, redirect: FJRouteCommonRedirector(redirect: { _ in "/info/display" }))
                ])
            ])
            await config.addRoute(r1)
            let r2 = try! FJRoute(path: "/info", builder: self._builder, routes: [
                FJRoute(path: "display", builder: self._builder, redirect: FJRouteCommonRedirector(redirect: { _ in "/login" }))
            ])
    ...
    @Test func testErrorMultipleRedirects() async throws {
        let config = await createConfig(action: { config in
            ...
            await config.addRoute(r1)
            let r2 = try! FJRoute(path: "/info", builder: self._builder, routes: [
                FJRoute(path: "display", builder: self._builder, redirect: FJRouteCommonRedirector(redirect: { _ in "/logine" }))
            ])
            await config.addRoute(r2)
            let r3 = try! FJRoute(path: "/login", builder: self._builder)
            await config.addRoute(r3)
    ...
    @Test func testLoopRedirect() async throws {
        let config = await createConfig(action: { config in
            ...
            await config.addRoute(r1)
            let r2 = try! FJRoute(path: "/info", builder: self._builder, routes: [
                FJRoute(path: "display", builder: self._builder, redirect: FJRouteCommonRedirector(redirect: { _ in "/user/settings/reset" }))
            ])
            await config.addRoute(r2)
        })
        let result = try await config.match(url: URL(string: "/user/settings/reset")!, extra: 123, ignoreError: false)
    ...
    @Test func testLoopRedirects() async throws {
        let config = await createConfig(action: { config in
            ...
            await config.addRoute(r1)
            let r2 = try! FJRoute(path: "/info", builder: self._builder, routes: [
                FJRoute(path: "display", builder: self._builder, redirect: FJRouteCommonRedirector(redirect: { _ in "/pkuser/details" }))
            ])
            await config.addRoute(r2)
            let r3 = try! FJRoute(path: "/pkuser/details", builder: self._builder, redirect: FJRouteCommonRedirector(redirect: { _ in "/pkuser/display" }))
            await config.addRoute(r3)
            let r4 = try! FJRoute(path: "/pkuser/display", builder: self._builder, redirect: FJRouteCommonRedirector(redirect: { _ in "/pkuser/pages/detail" }))
            ...
            let r5 = try! FJRoute(path: "/pkuser", builder: self._builder, routes: [
                FJRoute(path: "pages", builder: self._builder, routes: [
                    FJRoute(path: "detail", builder: self._builder, redirect: FJRouteCommonRedirector(redirect: { _ in "/info/display" }))
                ])
            ])
        ...
        let r2 = try! FJRoute(path: "/a", builder: _builder, routes: [
            try! FJRoute(path: "b", builder: _builder, routes: [
                try! FJRoute(path: "c", builder: _builder, routes: [
                    try! FJRoute(path: "/d", builder: _builder, redirect: FJRouteCommonRedirector(redirect: { _ in "/details" }))
                ])
            ])
        ])
        await config.addRoute(r2)
