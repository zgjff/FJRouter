...
struct FJRouterStoreMatchTests {
    ...
    @Test func testSuccessDoesNotRedirectNoChildHasRedirect() async throws {
        let config = await createConfig(action: { config in
            await config.addRoute(try! FJRoute(path: "/show/:id", builder: self._builder, interceptor: FJRouteCommonInterceptor(condition: { _ in false }, redirect: { _  in "/details" })))
        })
        let p = "/show/123"
        let url = URL(string: p)!
        let result = await config.match(url: url, extra: 123)
    ...
    @Test func testSuccessRedirectNoChildHasRedirect() async throws {
        let config = await createConfig(action: { config in
            await config.addRoute(try! FJRoute(path: "/show/:id", builder: self._builder, interceptor: FJRouteCommonInterceptor(condition: { _ in true }, redirect: { _  in "/pages/78" })))
        })
        let p = "/show/123"
        let url = URL(string: p)!
        let result = await config.match(url: url, extra: 123)
            ...
            let r = try! FJRoute(path: "/w", builder: _builder, routes: [
                try! FJRoute(path: "x", builder: _builder, routes: [
                    try! FJRoute(path: "y", builder: _builder, routes: [
                        try! FJRoute(path: "z", builder: _builder, interceptor: FJRouteCommonInterceptor(condition: { _ in true }, redirect: { _ in "/a/b/c" }))
                    ])
                ])
            ])
    ...
    @Test func testErrorHasChildsHasRedirect() async throws {
        let config = await createConfig(action: { config in
            let r = try! FJRoute(path: "/w", builder: _builder, routes: [
                try! FJRoute(path: "x", builder: _builder, routes: [
                    try! FJRoute(path: "y", builder: _builder, routes: [
                        try! FJRoute(path: "z", builder: _builder, interceptor: FJRouteCommonInterceptor(condition: { _ in true }, redirect: { _ in "/a/b/f" }))
                    ])
                ])
            ])
            await config.addRoute(r)
        ...
        let config = await createConfig(action: { config in
            let r1 = try! FJRoute(path: "/user", builder: self._builder, routes: [
                FJRoute(path: "settings", builder: self._builder, routes: [
                    FJRoute(path: "reset", builder: self._builder, interceptor: FJRouteCommonInterceptor(condition: { _ in true
                     }, redirect: { _ in "/info/display" }))
                ])
            ])
            await config.addRoute(r1)
            let r2 = try! FJRoute(path: "/info", builder: self._builder, routes: [
                FJRoute(path: "display", builder: self._builder, interceptor: FJRouteCommonInterceptor(condition: { _ in true
                }, redirect: { _ in "/login" }))
            ])
    ...
    @Test func testLoopRedirect() async throws {
        let config = await createConfig(action: { config in
            ...
            let r2 = try! FJRoute(path: "/info", builder: self._builder, routes: [
                FJRoute(path: "display", builder: self._builder, interceptor: FJRouteCommonInterceptor(condition: { _ in true
                }, redirect: { _ in "/user/settings/reset" }))
            ])
            await config.addRoute(r2)
        })
        let result = await config.match(url: URL(string: "/user/settings/reset")!, extra: 123)
    ...
    @Test func testLoopRedirects() async throws {
        let config = await createConfig(action: { config in
            ...
            let r2 = try! FJRoute(path: "/info", builder: self._builder, routes: [
                FJRoute(path: "display", builder: self._builder, interceptor: FJRouteCommonInterceptor(condition: { _ in true
                }, redirect: { _ in "/pkuser/details" }))
            ])
            await config.addRoute(r2)
            let r3 = try! FJRoute(path: "/pkuser/details", builder: self._builder, interceptor: FJRouteCommonInterceptor(condition: { _ in true
            }, redirect: { _ in "/pkuser/display" }))
            await config.addRoute(r3)
            let r4 = try! FJRoute(path: "/pkuser/display", builder: self._builder, interceptor: FJRouteCommonInterceptor(condition: { _ in true
            }, redirect: { _ in "/pkuser/pages/detail" }))
            ...
            let r5 = try! FJRoute(path: "/pkuser", builder: self._builder, routes: [
                FJRoute(path: "pages", builder: self._builder, routes: [
                    FJRoute(path: "detail", builder: self._builder, interceptor: FJRouteCommonInterceptor(condition: { _ in true
                     }, redirect: { _ in "/info/display" }))
                ])
            ])
        ...
        let r2 = try! FJRoute(path: "/a", builder: _builder, routes: [
            try! FJRoute(path: "b", builder: _builder, routes: [
                try! FJRoute(path: "c", builder: _builder, routes: [
                    try! FJRoute(path: "/d", builder: _builder, interceptor: FJRouteCommonInterceptor(condition: { _ in true }, redirect: { _ in "/details" }))
                ])
            ])
        ])
        await config.addRoute(r2)
