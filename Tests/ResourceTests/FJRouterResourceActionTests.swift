import Testing
import Foundation
@testable import FJRouter

struct FJRouterResourceActionTests {
    @Test func emptyPath() async throws {
        await #expect(throws: FJRouter.RegisterURIError.emptyPath) {
            let _ = try await FJRouterResource(uri: FJRouterCommonRegisterURI(path: ""), value: { _ in 1 })
        }
    }
    
    @Test func emptyName() async throws {
        await #expect(throws: FJRouter.RegisterURIError.emptyName) {
            let _ = try await FJRouterResource(uri: FJRouterCommonRegisterURI(path: "/", name: ""), value: { _ in 1})
        }
    }
    
    @Test func testEmptyParameters() async throws {
        let a1 = try await FJRouterResource(path: "/settings/detail/abc", value: { _ in 1})
        #expect(a1.pathParameters.isEmpty)
        let a2 = try await FJRouterResource(path: "/settings/detail/abc?ip=123", value: { _ in 1})
        #expect(a2.pathParameters.isEmpty)
    }
    
    @Test func testParamters() async throws  {
        let a1 = try await FJRouterResource(path: "/user/:id", value: { _ in 1})
        #expect(a1.pathParameters == ["id"])
        let a2 = try await FJRouterResource(path: "/user/:id/check/:age", value: { _ in 1})
        #expect(a2.pathParameters == ["id", "age"])
        let a3 = try await FJRouterResource(path: "/user/:id/check/:age/play/:game", value: { _ in 1})
        #expect(a3.pathParameters == ["id", "age", "game"])
        let a4 = try await FJRouterResource(path: "/user/:id/check/:age/play/:game?query?pid=1&gid=2", value: { _ in 1})
        #expect(a4.pathParameters == ["id", "age", "game"])
    }
    
    @Test func matchRegExpAsPrefixWithoutParameter() async throws {
        let pattern = "/settings/detail"
        let action = try await FJRouterResource(path: pattern, value: { _ in 1})
        
        #expect(action.matchRegExpHasPrefix("/settings/detail") != nil)
        #expect(action.matchRegExpHasPrefix("/settings/detail/abc") != nil)
        #expect(action.matchRegExpHasPrefix("settings/detail") != nil)
        #expect(action.matchRegExpHasPrefix("settings/detail/abc") != nil)
        #expect(action.matchRegExpHasPrefix("/settings/detail/") != nil)
        #expect(action.matchRegExpHasPrefix("settings/detail") != nil)
        #expect(action.matchRegExpHasPrefix("settings/detail/") != nil)
        #expect(action.matchRegExpHasPrefix("/settings/detail/") != nil)
        
        #expect(action.matchRegExpHasPrefix("/") == nil)
        #expect(action.matchRegExpHasPrefix("settings") == nil)
        #expect(action.matchRegExpHasPrefix("/settings") == nil)
        #expect(action.matchRegExpHasPrefix("/settings/Detail") == nil)
        #expect(action.matchRegExpHasPrefix("/settings/details") == nil)
        #expect(action.matchRegExpHasPrefix("settings/Detail") == nil)
        #expect(action.matchRegExpHasPrefix("settings/details") == nil)
    }
    
    @Test func testSameAction() async throws {
        let a1 = try await FJRouterResource(path: "/a", value: { _ in 1})
        let a2 = try await FJRouterResource(path: "/a", value: { _ in 1})
        #expect(a1 == a2)
    }
    
    @Test func testDifferentAction() async throws {
        let a1 = try await FJRouterResource(path: "/a", value: { _ in 1})
        let a2 = try await FJRouterResource(path: "a", value: { _ in 1})
        let a3 = try await FJRouterResource(path: "a/", value: { _ in 1})
        let a4 = try await FJRouterResource(path: "/a/", value: { _ in 1})
        let actions: Set<FJRouterResource> = [a1, a2, a3, a4]
        #expect(actions.count == 4)
    }
}
